 ...
 /**Builds clickable buttons to join an openGame*/
	function buildHTMLOpenGames(games) {
		...
		for (let i = 0; i < games.length && i < 5; i++) {
			...
			div.innerHTML = (`<span>ROOM ${i+1} <br> JOIN GAME</span>`);  /**iteration of Button content.*/ 
	  ...
	  /**We switch to the response code and handle accordingly.*/
	      switch (status) {
	        ...
	        case 200:
	          ...
	          location.href = "gameSurface-EN.html?gameId=" + game.id;
	          break;
	        case 500:
	        default:
	          console.log("Error 500: " + message);
		  ...
		  /**We switch to the response code and handle accordingly.*/
			switch (response.status) {
				...
				case 201:
					...
					location.href = "gameSurface-EN.html?gameId=" + gameId;
				break;
				case 400:
				case 401:
