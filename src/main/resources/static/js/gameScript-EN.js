...
function onError() {
    connectingElement.textContent = 'Could not create a connection to the WebSocket. Please refresh the page and try again!';
...
function onMessageReceived(payload) {
	...
	case("JOIN"): 	if (message.sender !== ownPlayer.account.username) {
					alert("Opponents found: "+ message.sender);
					}
	break;
	...
	case("CURRENT_PLAYER"): 
							...
							p.textContent = '"'+ message.content + '" is playing right now!';
	...
	case("BADREQUEST"):
					if (message.sender === ownPlayer.account.username) {
					alert("Dear player, if you have received this message, there are two possibilities:" +
					"\n1. You are not currently on" + 
					"\n2. You played an invalid card, see the rules in the help button.");
					}
	break;
	
	/**Message telling the round winner*/
	case("ROUNDwinner"):
					alert("Round won by: "+ message.content);
					getCurrentGame();
	...
	case("LEAVE"):
					if (endedGame) {
						alert(message.sender + "Game over, you return to the lobby.");
						window.location.href='lobby-EN.html';
					} else {
						alert(message.sender + " has left the game. The game is deleted.");
						window.location.href='lobby-EN.html';
					}
	break;
	...
	case("STARTPHASE2"):
					alert("Phase 2 is initialised!");
	break;
	
	/**Message telling the phase winner*/
	case("GAMEWinner"):
					alert("Game won by: " + message.content);
					deleteGame();
	break;
	
	/**Message for player to wait for an opponent*/
	case("WAIT"):
					alert("Wait for a player to log in!")
	break;
	}
}
